import { NavLink, Outlet } from "react-router-dom";
import { logout } from "../api";
import { LogOut } from "lucide-react";

export default function TeamLeadLayout() {
  return (
    <div className="w-full h-full relative overflow-hidden flex">
      <div className="bg-glow bg-glow-1"></div>
      <div className="bg-glow bg-glow-2"></div>

      <div className="w-full h-full grid grid-cols-[240px_1fr] p-4 gap-4 z-10 relative">
        <aside className="sidebar glass-panel flex flex-col py-5 px-3 gap-6">
          <div className="flex items-center gap-3 px-3">
            <div
              className="w-5 h-5 rounded-[4px] shadow-[0_0_10px_var(--accent-purple-glow)]"
              style={{
                background:
                  "linear-gradient(135deg, var(--accent-purple-light), var(--accent-purple))"
              }}
            ></div>
            <h3 className="font-medium text-[15px] tracking-tight text-white">
              Nexus
            </h3>
          </div>

          <div className="flex flex-col gap-1 mt-2">
            <span className="label-caps px-3 mb-2">Team Lead</span>
            <NavItem to="/team-lead/team-overview" label="Team Overview">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path>
                <circle cx="9" cy="7" r="4"></circle>
                <path d="M22 21v-2a4 4 0 0 0-3-3.87"></path>
                <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
              </svg>
            </NavItem>
            <NavItem to="/team-lead/sprint-board" label="Sprint Board">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                <line x1="16" y1="2" x2="16" y2="6"></line>
                <line x1="8" y1="2" x2="8" y2="6"></line>
                <line x1="3" y1="10" x2="21" y2="10"></line>
              </svg>
            </NavItem>
            <NavItem to="/team-lead/sprint-retro" label="Sprint Retro">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"></polyline>
              </svg>
            </NavItem>
            <NavItem to="/team-lead/backlog" label="Backlog">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <line x1="8" y1="6" x2="21" y2="6"></line>
                <line x1="8" y1="12" x2="21" y2="12"></line>
                <line x1="8" y1="18" x2="21" y2="18"></line>
                <line x1="3" y1="6" x2="3.01" y2="6"></line>
                <line x1="3" y1="12" x2="3.01" y2="12"></line>
                <line x1="3" y1="18" x2="3.01" y2="18"></line>
              </svg>
            </NavItem>
            <NavItem to="/team-lead/analytics" label="Analytics">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <line x1="18" y1="20" x2="18" y2="10"></line>
                <line x1="12" y1="20" x2="12" y2="4"></line>
                <line x1="6" y1="20" x2="6" y2="14"></line>
              </svg>
            </NavItem>
          </div>

          <div className="flex flex-col gap-1 mt-auto">
            <span className="label-caps px-3 mb-2">System</span>
            <NavItem to="/team-lead/notifications" label="Notifications">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"></path>
                <path d="M13.73 21a2 2 0 0 1-3.46 0"></path>
              </svg>
            </NavItem>
            <NavItem to="/team-lead/settings" label="Settings">
              <svg
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              >
                <circle cx="12" cy="12" r="3"></circle>
                <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path>
              </svg>
            </NavItem>
            
            <div className="mt-4 pt-4 border-t border-white/10">
              <button 
            className="flex items-center gap-3 w-full px-4 py-3 rounded-xl transition-all duration-200 text-red-400 hover:text-red-300 hover:bg-red-500/10 active:scale-95 group"
            onClick={logout}
            id="sidebar-logout-btn"
          >
            <div className="flex items-center justify-center w-5 h-5 transition-transform group-hover:-translate-x-1">
              <LogOut size={20} strokeWidth={2.5} />
            </div>
            <span className="font-semibold text-sm tracking-wide uppercase">Logout Properly</span>
          </button>
            </div>
          </div>
        </aside>

        <main className="flex flex-col gap-4 overflow-hidden h-full">
          <Outlet />
        </main>
      </div>
    </div>
  );
}

function NavItem({ to, children, label }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `nav-item ${isActive ? "active" : ""}`
      }
    >
      {children}
      {label}
    </NavLink>
  );
}
