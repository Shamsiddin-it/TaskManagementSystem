...
public class JoinRequestController(IJoinRequestService service) : ControllerBase
{
    ...
    [HttpPut("{id}")]
    public async Task<Response<string>> UpdateAsync(int id, UpdateJoinRequestDto dto) => await _service.UpdateAsync(id, dto);
    ...
    [HttpGet("{id}")]
    public async Task<Response<GetJoinRequestDto>> GetByIdAsync(int id) => await _service.GetByIdAsync(id);
}
